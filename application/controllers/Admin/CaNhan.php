...
class CaNhan extends CI_Controller {
	...
	public function index()
	{
		...
		$data['detail'] = $this->Model_NguoiDung->getById($this->session->userdata('manguoidung'));
		if ($this->input->server('REQUEST_METHOD') === 'POST') {
			$hoten = $this->input->post('hoten');
			$email = $this->input->post('email');
			$sodienthoai = $this->input->post('sodienthoai');
			$taikhoan = $this->input->post('taikhoan');
			$matkhau = $this->Model_NguoiDung->getById($this->session->userdata('manguoidung'))[0]['MatKhau'];
			$anhchinh = $this->Model_NguoiDung->getById($this->session->userdata('manguoidung'))[0]['AnhChinh'];
			...
			$this->Model_NguoiDung->update($hoten,$anhchinh,$taikhoan,$matkhau,$email,$sodienthoai,$this->session->userdata('manguoidung'));
